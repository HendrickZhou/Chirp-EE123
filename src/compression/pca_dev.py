...
def find_sections():
    ...
    for r in range(nRow-1):
        rowCorr=np.corrcoef(image_stack[:,r,:,:].flatten(),image_stack[:,r+1,:,:].flatten())
    ...
    for r in range(nCol-1):
        colCorr=np.corrcoef(image_stack[:,:,r,:].flatten(),image_stack[:,:,r+1,:].flatten())
    ...
    for ri in range(nRowSec):
        for ci in range(nColSec):
            ...
            for img in range(nImages):
                r=image_stack[img,rowSecStart:rowSecEnd,colSecStart:colSecEnd,0].flatten()
                g=image_stack[img,rowSecStart:rowSecEnd,colSecStart:colSecEnd,1].flatten()
                b=image_stack[img,rowSecStart:rowSecEnd,colSecStart:colSecEnd,2].flatten()
...
def procInput_noFlatten():
    ...
    for ri in range(nRowSec):
        for ci in range(nColSec):
            ...
            for img in range(nImages):
                r = image_stack[img, rowSecStart:rowSecEnd, colSecStart:colSecEnd, 0]
                g = image_stack[img, rowSecStart:rowSecEnd, colSecStart:colSecEnd, 1]
                b = image_stack[img, rowSecStart:rowSecEnd, colSecStart:colSecEnd, 2]
